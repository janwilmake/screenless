# Self-hosting screenless

Everything you need to run your own screenless Worker and CLI. Most people never
open this file — the hosted service at [screenless.sh](https://screenless.sh) is
the default. Read it if you want to run the backend yourself, or understand how
the pieces fit.

## How it fits together

```
  CLI + skills  ─────►  Cloudflare Worker  ─────►  Telnyx
  (your machines)       (D1 + R2 + secrets)     (verify, calls, SMS)
        ▲                       │
        │  screenless watch     │  status + conversation webhooks
        └───────────────────────┘
```

- **The API key lives in the Worker, never the CLI.** The CLI holds only a
  session token bound to a verified phone number.
- **Your phone number is your identity.** `screenless setup` sends an OTP;
  entering it mints the session. No password, no OAuth.
- **It only ever dials verified numbers on your own team.** A self-call takes
  `To` from the session token; a teammate call resolves targets to your org's
  verified members. The number is never a free-form request value — which keeps
  this structurally incapable of becoming a dialer (see Legal, below).
...
skills/                    the branded skills — installable via skills.sh
  screenless/                the watcher: takes incoming calls and acts on them
    SKILL.md, APPLY.md         the watcher, and the return leg after a call
  call-when-afk/             phone the user their questions while away
  morning-pr-review/         the optional nightly loop: brief + weekly paper
    SKILL.md
    press/                     the PDF toolkit the paper skill calls
site/src/                  the landing page and installer (screenless.sh)
demo.gif                   the hero terminal animation
```
