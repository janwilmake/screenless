...
|---|---|
| [Session 9 — teams, pay-as-you-go, and the watcher](2026-08-21-session-9-teams-payg-and-the-watcher.md) | The single-person subscription becomes team infrastructure: orgs and users in D1, email-only invites on one page at screenless.sh/team, billing per org at $10 free then ~30¢/min. Ring-ins get a robot menu — press 1 for the assistant, or speak a request that is transcribed and routed to whichever teammate's terminal runs `screenless watch` (own terminal first, queue up to a week when none). Also: the two-half-circle logo, every email branded HTML, and the two Workers merged into one on the apex. |
