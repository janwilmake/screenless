...
- 20:2x — `i just had a call ( i called proactively  to the dutch phone ). the shell wait didnt come back yet. can u check if the transcript is in already? is something wrong or is it just slow?`
  *(wrong: a ring-in never reported its end — no status callback on the inbound TeXML — so the record sat at "answered" and the loop never woke. Built: `/webhooks/inbound/status` matched by caller number, set as the Call progress events URL on the `screenless-inbound` TeXML app in the portal; an hourly stale sweep; an admin finish, used to close this call. ADMIN_SECRET rotated, kept in `~/.screenless/.admin-secret`. The call itself — 14½ minutes, the new three-item brief — went as intended; three decisions applied as PR comments, one research ticket opened (HYR2-972), report mailed.)*
- 20:32 — `park a fresh brief with the remaining two for 08:00`
  *(reversed one minute later by the next prompt; never acted on. Recovered from the transcript by the intent seed scanner in session 7 — the hand-written record had missed it)*
- 20:33 — `no dont call me tomorrow . this is great. thanks! bye, gtg now, save my intent`
  *(no call on 20 August: the 08:00 brief is already `placed` from the ring-in, so nothing dials; the paper is still queued for 07:45. Tonight is stamped; the waiter's next NIGHTLY is 03:00 on 21 August)*
